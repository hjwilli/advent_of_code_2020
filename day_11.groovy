...
def part1(def input) {
	...
	return cur.flatten().findAll{ it == "#" }.size()
}
...
def seatGame(def input) {
	// deep copy
	def res = []
	input.each { res << it.collect() }

	def occupied

	def rowMax = input.size()-1
	def colMax = input.first().size()-1

	// iterate through seats
	for ( r in (0..rowMax ) ) {
		for ( c in (0..colMax ) ) {
			count = 0 // count of occupied neighbors

			// iterate through adjacent seats
			for (i in (-1..1)) {
				for (j in (-1..1)) {
					adjRow = r + i
					adjCol = c + j
					if ( ( adjRow >=0 ) && (adjRow <= rowMax) &&
						 ( adjCol >=0 ) && (adjCol <= colMax) &&
						 !( i==0 && j==0)
						) {
						if ( input[adjRow][adjCol] == "#" ) count++

						//if (r == 9) println "seat[$r][$c], checking [$adjRow][$adjCol]"
					}
				}
			}

			//if (r == 9) println "seat[$r][$c] count: $count, val:${input[r][c]}"

			// update seat
			if ((input[r][c] == "L") && count == 0) res[r][c] = "#"
			if ((input[r][c] == "#") && count >= 4) res[r][c] = "L"
		}
	}
...
//seat game for part 2
def seatGame2(def input) {
	// deep copy
	def res = []
	input.each { res << it.collect() }
	...
	def vectors = [] // directional vectors
	for (i in (-1..1)) {
		for (j in (-1..1)) {
			if (!(i==0 && j==0)) vectors << [i, j]
		}
	}
	...
	for ( r in (0..rowMax ) ) {
		for ( c in (0..colMax ) ) {
			def count = 0 // count of occupied neighbors
			...
			for (v in vectors) {
				...
				while ( validSeat(adjRow, adjCol) && !found ) {
					if ( input[adjRow][adjCol] != "." ) found = true
					if ( input[adjRow][adjCol] == "#" ) count++
			...
			// update seat
			if ((input[r][c] == "L") && count == 0) res[r][c] = "#"
			if ((input[r][c] == "#") && count >= 5) res[r][c] = "L"
		}
	}
...
L.LLLLL.LL
'''
	return strToListOfLists(inputStr)
}
...
def strToListOfLists(def input) {
	def res = []

	input.eachLine { line ->
		res << line.trim().toCharArray().collect()
	}
