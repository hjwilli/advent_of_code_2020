...
def part1(def input) {
	...
	input.eachLine { line ->
		//println "line: $line"
		if (line.trim().size() == 0) {
			counts << evalGroup(group)
			group = ""
		}
		else group = group + line.trim()
	}
...
def part2(def input) {
	...
	input.eachLine { line ->
		if (line.trim().size() == 0) {
			...
			group = line.trim().toSet() 
			reset = false
		}
		else group = group.intersect(line.trim().toSet())
	}
	//println counts
	//println groups
...
def evalGroup(def input) {
	//print input
	//println " - " + input.toSet().size()
