...
def part_1(def input) {
	...
	input.each { min, max, test, string ->
		def res = string.findAll(test)
		if (res.size() < min || res.size() > max) {
			println "fail: $min - $max, $test, $string, $res.size"
			fail_count = fail_count + 1
		}
		else {
			println "pass: $min - $max, $test, $string, $res.size"
...
def part_2(def input) {
	...
	input.each { min, max, test, string ->
		def safe_max = max >= string.size() ? string.size() : max
		def substr = string.substring(min-1, safe_max)

		println "$min - $max, $test, $string, $substr"

		def test_min = string.getAt(min-1).equals(test)
		def test_max = max > string.size() ? false : string.getAt(max-1).equals(test)
