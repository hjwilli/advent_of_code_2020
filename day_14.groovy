...
def part_1(program) {
	...
	program.each {cmd, val ->
		println ("RUN $cmd, $val")
		if (cmd == "mask") mask = val
		if (cmd == "mem") apply_mask(mask, val[0], val[1])
	}
}
...
def part_2(program) {
	...
	program.each {cmd, val ->
		println ("RUN $cmd, $val")
		if (cmd == "mask") mask = val
		if (cmd == "mem") apply_mem_mask(mask, val[0], val[1])
	}
}

def apply_mask(def mask, def id, def num) {
	
	def orMask  = Long.parseLong(mask.replaceAll("X", "0"), 2)
	def andMask = Long.parseLong(mask.replaceAll("X", "1"), 2)

	//println ("andMask: ${Long.toBinaryString(andMask)}")
	//println ("orMask: ${Long.toBinaryString(orMask)}")

	println "\nnum: ${Long.toBinaryString(num)}"
	def masked_num = num & andMask
	println "masked_num: ${Long.toBinaryString(masked_num)} - $masked_num"
	masked_num = masked_num | orMask
	println "masked_num: ${Long.toBinaryString(masked_num)} - $masked_num"
	res[id] = masked_num
}

def apply_mem_mask(def mask, def id, def num) {
	def orMask  = Long.parseLong(mask.replaceAll("X", "0"), 2)

	def masked_id = id | orMask
	println "\nid: ${Long.toBinaryString(id)}"
	println "masked_id: ${Long.toBinaryString(masked_id)} - $masked_id"

	def floaters = []
	mask.eachWithIndex {val, idx -> if (val == "X") floaters << idx }
	
	write_masked_val(floaters, masked_id, num)
}


def write_masked_val(def floaters, def id, def num) {
	//println ("wmv: $floaters, $id, $num")

	if (!floaters) {
		//println ("setting ${Long.toBinaryString(id)} - $id")
		res[id] = num as Long
		return
	}
	...
	andMask[i] = 0
	orMask[i] = 1
	andMask = Long.parseLong(andMask.join(), 2)
	orMask = Long.parseLong(orMask.join(), 2)

	write_masked_val(floaters.clone(), (id & andMask), num)
	write_masked_val(floaters.clone(), (id | orMask), num)
}
