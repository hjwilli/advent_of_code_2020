...
def part_1(def input) {
	...
	input.eachLine { line ->
		//println "line: $line"
		if (line.trim().size() == 0) {
			if (evalPassport(passport)) valid = valid + 1
			passport = ""
		}
...
def part_2(def input) {
	...
	input.eachLine { line ->
		//println "line: $line"
		if (line.trim().size() == 0) {
			if (evalValidatePassport(passport)) valid = valid + 1
			passport = ""
		}
		else passport = passport + line.trim() + " "
	}
...
def evalPassport(def passport) {
	...
	required.each { search ->
		valid = valid && passport.contains(search)
	}
...
def evalValidatePassport(def passportStr) {
		def required = [
		"byr",
		"iyr",
		"eyr",
	...
	def valid = true
	//println passportStr
	passportStr = passportStr.trim()
	passportStr = passportStr.replaceAll(" ", '", "')
	passportStr = passportStr.replaceAll(":", '":"')
	passportStr = '"' + passportStr + '"'
	//println passportStr

	def p = evaluate("""return [$passportStr] """)
	...
	*/
	...
	if (!(p.containsKey("byr") && p.byr as int >= 1920 && p.byr as int <= 2002 ))
		return false
	if (!(p.containsKey("iyr") && p.iyr as int >= 2010 && p.iyr as int <= 2020 ))
		return false
	if (!(p.containsKey("eyr") && p.eyr as int >= 2020 && p.eyr as int <= 2030 ))
		return false
	if (!(p.containsKey("hgt")))
		return false

	def hgtVal = p.hgt.find('^([0-9]*)')
	if (p.hgt.contains("cm")) {
		if (!(hgtVal as int >=150 && hgtVal as int <=193))
			return false
	}
	else if (p.hgt.contains("in")) {
		if (!(hgtVal as int >=59 && hgtVal as int <=76))
			return false
	}
	else return false

	if (!(p.containsKey("hcl") && p.hcl.find('^#[abcdef0123456789]{6}$')))
		return false
	if (!(p.containsKey("ecl") && ["amb", "blu", "brn", "gry", "grn" ,"hzl", "oth"].contains(p.ecl)))
		return false
	if (!(p.containsKey("pid") && p.pid.find('^[0-9]{9}$')))
		return false
