...
// ---
def part1(def input) {
	def rules = parseInput( input )
	def algToFoods = getPossibleAllergens( rules )
 
	def possibleBadFoods = algToFoods.values().flatten()

	def count = 0
	rules.each { foods, _ ->
		for (food in foods) {
			if ( !possibleBadFoods.contains(food) ) count = count + 1
		}
	}
...
def parseInput( input ) {
	...
	input.eachLine { line ->
		if (line.trim().size() == 0) return
		def (foods, algs) = line.trim().substring( 0, line.size() - 1 ).split(" \\(contains ")
		
		rules << [foods.split(" ")*.trim(), algs.split(",")*.trim()]
	}

	return rules
}

// get set of possible foods that can go to an allergen
// return [alg : Set [foods]]
def getPossibleAllergens ( rules ) {
	def algToFoods = [:]

	rules.each { foods, algs ->
		for ( a in algs ) {
			if ( !algToFoods.keySet().contains(a) ) algToFoods[a] = foods as Set
			else algToFoods[a] = algToFoods[a].intersect(foods as Set)
		}
	}
...
def part2(def input) {
	...
	def lookup = getFinalAlergens( algToFoods )
	//println lookup

	def res = ""
	lookup.keySet().sort().each {a -> 
		res = res + lookup[a] + ","
	}
...
def getFinalAlergens( def algToFoods ) {
	println algToFoods

	def algSize = algToFoods.size()

	// alg -> food
	def solution = [:]
	def found = [] // food on the bad list

	while ( solution.size() < algSize ) {
		algToFoods.each {a, f -> 
			if (f.size() == 1 ) {
				solution[a] = f.first()
				found << f.first()
		...
		algToFoods.each { a, f -> f.removeAll(found) }
	}
